...
absl::Status DelegateKernel::Initialize(
  ...
  // Gate: when use_ir_model is set, run the IrModel pipeline instead of the
  // legacy GraphFloat32 pipeline below. Default is false.
  ...
  ABSL_RETURN_IF_ERROR(
      UpdateCreateInfoWithExternalTensors(context, create_info));
...
// ===========================================================================
// IrModel pipeline (gated by options->use_ir_model). Mirrors the GraphFloat32
// pipeline above but operates on ::ml_drift::ir::IrModel. Kept separate so the
// legacy GraphFloat32 path is unchanged; delete this block once the migration
// is complete.
// ===========================================================================
absl::Status DelegateKernel::InitializeIrModel(
  ...
  for (size_t i = 0; i < ir_inputs.size(); ++i) {
    ...
    if (producer != nullptr || !consumers.empty()) {
      ...
      input_refs.push_back(non_const_input_refs[i]);
    }
  }
  ...
  for (size_t i = 0; i < output_size; ++i) {
    ...
    if (producer != nullptr || !consumers.empty()) {
      ...
      output_refs.push_back(output_tensors_refs[i]);
    }
  }
  ...
  if (delegate_data_->options->enable_op_profiling_detailed_report) {
    ABSL_LOG(INFO) << create_info.external_mutable_tensors.size() << std::endl;
    ::ml_drift::ProfilingInfo profiling_info;
    ABSL_RETURN_IF_ERROR(ctx_->Profile(profiling_info));
