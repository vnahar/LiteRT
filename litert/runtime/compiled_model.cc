...
LiteRtCompiledModelT::Create(LiteRtEnvironmentT* env, LiteRtModel model,
...
#if defined(LITERT_ENABLE_FABRIC_INTEGRATION)
  {
    ...
    if (auto fabric_options = litert::FindOpaqueData<LiteRtFabricOptionsT>(
        ...
        fabric_options) {
      if (jit_compilation_options->selected_signature_keys.has_value()) {
        return Unexpected(
...
Expected<void> LiteRtCompiledModelT::InitializeActiveSubgraphs(
  ...
  selected_signature_keys_.reset();

  if (options == nullptr || !options->selected_signature_keys.has_value()) {
  ...
  selected_signature_keys_.emplace();
...
Expected<void> LiteRtCompiledModelT::ValidateSignatureIsActive(
  ...
  if (!selected_signature_keys_.has_value() ||
      selected_signature_keys_->contains(std::string(signature_key))) {
    return {};
  }
  return Unexpected(
