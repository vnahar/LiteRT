...
RunResult RunReduceCase(
    const ReduceCase& test_case, KernelVariant kernel_variant,
...
std::vector<int32_t> MakeXnnpackAxes(const std::vector<int32_t>& axis_values,
  ...
  for (int32_t axis_value : axis_values) {
    ...
    if (axis < 0) axis += rank;
    if (absl::c_find(axes, axis) == axes.end()) {
      axes.push_back(axis);
...
ReduceCase MakeXnnpackReduceCase(const XnnpackReduceSpec& spec) {
  ...
  for (int32_t i = 0; i < rank; ++i) {
    const int32_t dim = i < static_cast<int32_t>(spec.input_shape.size())
